...
overheatedLights = ["top-left","top-right","top-front","top-back","bottom-front","bottom-back"]
...
def paintLines(frame):
    if frame >0:
        if frame < 20:
            set_pixel_line_gradient(0,max(frame,0),[255,80,0],[0,255,150],lights['heat-left'])
            set_pixel_line_gradient(0,max(frame,0),[255,80,0],[0,255,150],lights['heat-right'])
        else:
            set_pixel_line_gradient(max(frame-20,0),24,[255,80,0],[0,255,150],lights['heat-left'])
            set_pixel_line_gradient(max(frame-20,0),24,[255,80,0],[0,255,150],lights['heat-right'])
    else:
...
class Firing(State):
    ...
    def enter(self,sm):
        ...
        lights['top-back'].animateColor(config['colors']['hot'],7)
    def exit(self,sm):
        ...
        lights['top-back'].animateColor(config['colors']['normal'],10)
        sm.heat = sm.heat + 2
        if sm.frame < 20:
            sm.frame +=2
        else:
